...
When a validator reports `504 Gateway Timeout`, `get chain ID`, `validator is banned`, `NO_PROVIDER_FOR_PROMPT`, `DEGRADED`, or Morpheus `model_unavailable` / HTTP 503 errors, triage before changing configuration:
...
1. **Classify the symptom.**
   ...
   - Morpheus `model_unavailable` or HTTP 503 responses can mean the hosted gateway catalog changed even when the API key is valid and the release YAML is unchanged. Do not assume a fresh node is misconfigured; identify working models, disable unavailable Morpheus model entries, or switch to another supported provider before putting the node in rotation.
...
- **LLM Provider**: API key from one of these providers:
  ...
  - Anthropic
  - Google (Gemini)
  - Morpheus (hosted model availability can change; verify the configured models before relying on it)
...
- LLM provider setup (Heurist, Comput3, io.net, LibertAI, Anthropic, Google, Morpheus)
